...
# ---------------------------------------------------------------------------
# Provider switcher (Anthropic, z.ai, NVIDIA, ...) - settings.json merge system
# ---------------------------------------------------------------------------
...
# <- providers/<active>.json. A real file, not a symlink, so it works on Windows
# where symlinks need admin/Developer Mode. Provider files are local
# (gitignored, hold auth tokens); *.json.example templates are tracked with a
# <ZAI_TOKEN>-style placeholder. `ccs` (-> bin/cc-provider.ps1) performs the
# merge and records the choice in providers/.active.
...
function Set-Provider {
    ...
    # settings.base.json. Seeding only writes settings.json when it does not
    # already exist, so on a machine that has ever run `ccs` nothing would
    # otherwise put the repo-owned safety block back - which is exactly how the
    # docker-volume deny rules and both safety hooks went missing while still
    # sitting in git. Re-running the merge here repairs those installs.
    $activeFile = Join-Path $pdir '.active'
    $switcher = Join-Path $ClaudeDir 'bin/cc-provider.ps1'
    if ((Test-Path -LiteralPath $activeFile) -and (Test-Path -LiteralPath $switcher)) {
        $active = (Get-Content -LiteralPath $activeFile -Raw).Trim()
        if ($active -and (Test-Path -LiteralPath (Join-Path $pdir "$active.json"))) {
            try {
                & $switcher $active | Out-Null
                Write-Step "Re-applied provider '$active' so settings.json carries the current safety config"
            } catch {
                Write-Warning "could not re-apply provider '$active'; run: ccs $active"
            }
        }
    }
