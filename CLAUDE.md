...
## This repository — non-obvious invariants

Full documentation lives in [`docs/`](docs/README.md); this section is only the
...
- **Never hand-edit `catalog/generated/**` or `docs/skills/**`.** They are
  regenerated by `bun run catalog:generate`; CI fails if the committed output
  differs from a fresh build. Change the generator or the source, then regenerate.
- **`install.sh` and `install.ps1` must stay at the repository root.** Their raw
  githubusercontent URLs are the documented public install commands.
- **GitHub Release asset names are a compatibility contract.** Files may move in
  the tree; the published names may not change. The release workflow stages
  assets under their public names.
- **`skills-sources.toml` and `install.sh` must agree.** `catalog/src/parity.ts`
  fails the build if they drift — update both together.
- **Catalog generation must stay deterministic.** No timestamps, no unordered
  map iteration, no locale-dependent sorting in generated output.
- **Never weaken the merge gate.** No `--admin`, no ruleset bypass, no
  direct-merge fallback. A change that adds capability surface must stay
  reviewable; see [`docs/security-model.md`](docs/security-model.md).
- Adding a capability to `SecurityProfile` requires adding it to
  `ESCALATION_CAPABILITIES` — a test enforces this, because a missed entry
  silently widens what auto-merges.
