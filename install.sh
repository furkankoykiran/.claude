...
# ---------------------------------------------------------------------------
seed_configs() {
  local target="$CLAUDE_DIR/config.json"
  if [ ! -f "$target" ] && [ -f "$CLAUDE_DIR/config.json.example" ]; then
    cp "$CLAUDE_DIR/config.json.example" "$target"
    chmod 600 "$target"
    log "Seeded config.json from config.json.example (edit it with your values)"
  fi

  # settings.json is the repo-owned safety config plus the optional example,
  # never one or the other. settings.base.json holds what must always be
  # present; settings.json.example holds only the optional extras and
  # deliberately does not repeat the safety block, so the two cannot drift.
  target="$CLAUDE_DIR/settings.json"
  local base="$CLAUDE_DIR/settings.base.json"
  local example="$CLAUDE_DIR/settings.json.example"
  if [ ! -f "$target" ] && [ -f "$base" ]; then
    if command -v jq >/dev/null 2>&1; then
      if jq -s '.[0] * (.[1] // {}) | del(._comment) | del(.["$comment"])' \
           "$base" "$example" > "$target.seed" 2>/dev/null; then
        chmod 600 "$target.seed"
        mv -f "$target.seed" "$target"
        log "Seeded settings.json from settings.base.json + settings.json.example"
      else
        rm -f "$target.seed"
        warn "could not seed settings.json; run 'ccs <provider>' once jq is available"
      fi
    else
      # No jq: ship the safety config alone rather than nothing. The optional
      # extras are a convenience; the deny rules and hooks are not.
      cp "$base" "$target"
      chmod 600 "$target"
      warn "jq not found — seeded settings.json from settings.base.json only (no optional extras)"
    fi
  fi
}
...
# ---------------------------------------------------------------------------
# 6. Provider switcher (Anthropic, z.ai, NVIDIA, ...) — settings.json merge system
# ---------------------------------------------------------------------------
# settings.json is GENERATED by merging, lowest precedence first: the existing
# settings.json (carried over) <- settings.base.json (repo-owned safety config)
# <- providers/<active>.json. A real file, not a symlink, so it also works on
# Windows where symlinks need admin/Developer Mode. Provider files are local
# (gitignored, hold auth tokens); *.json.example templates are tracked with a
# <ZAI_TOKEN>-style placeholder. `ccs` (-> bin/cc-provider on Unix,
# bin/cc-provider.ps1 on Windows) performs the merge and records the choice in
# providers/.active. Fresh installs opt in by running `ccs <provider>` once
# after filling the token.
#
# It is a merge rather than a copy because a copy silently deleted the
# repo-owned deny rules and safety hooks on every switch: no provider template
# carries them, and seed_configs() only writes settings.json when it is absent.
#
...
setup_providers() {
  ...
  # Re-apply the active provider so settings.json is rebuilt from the current
  # settings.base.json. seed_configs() only writes settings.json when it does
  # not already exist, so on a machine that has ever run `ccs` nothing would
  # otherwise put the repo-owned safety block back — that is exactly how the
  # docker-volume deny rules and both safety hooks went missing while still
  # sitting in git. Re-running the merge here repairs those installs on upgrade.
  if [ -f "$pdir/.active" ] && [ -x "$CLAUDE_DIR/bin/cc-provider" ]; then
    local active
    active="$(tr -d '[:space:]' < "$pdir/.active")"
    if [ -n "$active" ] && [ -f "$pdir/$active.json" ]; then
      if "$CLAUDE_DIR/bin/cc-provider" "$active" >/dev/null 2>&1; then
        log "Re-applied provider '$active' so settings.json carries the current safety config"
      else
        warn "could not re-apply provider '$active'; run 'ccs $active' once jq is available"
      fi
    fi
  fi
